...
# при установке пакета возникла проблема файл с шаблоном базы не появился в том вместе где должен был быть по мануалу 
...
# скриптом заливаем шаблон базы в новую базу из папки с исходниками
ADD data.sh /
RUN bash -c "/data.sh"
...
# скачиваем бинарник который поможет запустить несколько сервисов в докер контейнере 
...
# делаем линк где лежит фронтенд в папку www/html/ 
RUN ln -s usr/share/zabbix /var/www/html/

# для запуска забикс сервера нужно создать папку и сделать пользователя забикс нее владельцем 
